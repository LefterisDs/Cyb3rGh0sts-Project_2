...
Line htpasswd[100];      // contents of www/htpasswd, max 100 entries, format is <username>:<password-md5>
...
int main(int c, char **v) {
  read_file("www/htpasswd", htpasswd, 100);
  serve_forever("9000");
  return 0;
}
...
void route() {
  ...
  ROUTE_POST("/ultimate.html") {
    ...
    Line admin_pwd[1];
    read_file("www/admin_pwd", admin_pwd, 1);
