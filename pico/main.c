...
int main(int c, char **v) {
    ...
    fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> HTPASSWD %s \033[0m\n", htpasswd[0]);
...
void route() 
{
    ...
    fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECKPOINT ROUTE \033[0m\n");

    header_t *h = request_headers();
    while (h->name && strcmp(h->name, "Authorization") != 0) {
        fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> H->NAME %s \033[0m\n", h->name);
        fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> H->VAL  %s \033[0m\n", h->value);
        h++;
    }
    if(h->name) {
        fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECKPOINT ROUTE 1 \033[0m\n");
        ...
        if(!check_auth(htpasswd, h->value))
            return;
        fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECKPOINT ROUTE 2 \033[0m\n");

    } else {
        fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECKPOINT ROUTE 3 \033[0m\n");
...
int check_auth(Line users[], char *auth_header) 
{
    ...
    fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECK AUTH HEADER %s \033[0m\n", auth_header);
    ...
    fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECK AUTH PASSWD   %s \033[0m\n", auth_password);
    ...
    for(int i = 0; strcmp(users[i], "") != 0; i++) {
        fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECK AUTH CHECHPOINT 1\033[0m\n");
        fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECK AUTH USER     %s \033[0m\n", users[i]);
        if(strncmp(users[i], auth_username, ul) == 0 && users[i][ul] == ':') {
            password_md5 = users[i] + ul + 1; // <md5> part, after the ':'
            fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECK AUTH PASS MD5 %s \033[0m\n", password_md5);
            break;
        }
    }

    fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECK AUTH CHECHPOINT 2\033[0m\n");
    // check if user is found
    if(password_md5 == NULL) {
        fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECK AUTH CHECHPOINT 3\033[0m\n");
        printf("HTTP/1.1 401 Unauthorized\r\n");
        printf("WWW-Authenticate: Basic realm=\"");
        printf("Invalid user: ");
    ...
    fprintf(stderr,"\033[94m >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> CHECK AUTH CHECHPOINT 4\033[0m\n");
...
void serve_index() {
    printf("HTTP/1.1 200 OK\r\n\r\n");
    send_file("/mnt/c/Users/Michael/Desktop/CompSec2/pico/www/index.html");
}

void serve_ultimate() {
    printf("HTTP/1.1 200 OK\r\n\r\n");
    send_file("/mnt/c/Users/Michael/Desktop/CompSec2/pico/www/ultimate.html");
}
